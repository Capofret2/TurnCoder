...
1. **那条「减 1」的例外已经不适用了，别再减。** `libs/editops.js` 曾有一个 `U+25A0 ■` 当图例色块用（`<span style="color:...">■</span>` 后接分类名与占比），与展开折叠无关，所以按几何图形区（U+25A0–U+25FF）机械扫描会得到 34 个、需要减掉它才是箭头的 33 个。

   **构成图重写为横向条时那个色块换成了纯文字标签，`■` 的计数现在是 0**，因此机械扫描直接得 33。留着这段是因为一条减法指令比没有说明更危险：照它做会报出 32。

   同一次实测顺带交叉验证了下面那张表，逐文件与逐码点都与记载一字不差（`chat.js` 24、`frontend.html` 6、`models.js` 2、`panels.js` 1；`▶` 14、`▼` 8、`▾` 8、`▴` 3）。所以那张表可以直接信任，不需要重新数一遍。
