...
def _apply_planned_cost_replacements(text):
    """Apply the planned-tool wording swaps to one cost-rules text block.

    Both the Anthropic and non-Claude injection paths build the same
    [用户计费规范] text and then apply the same rewrites when planned tools
    are enabled. Centralising the list eliminates the two copies that have
    already drifted apart once.
    """
    for _old, _new in _PLANNED_COST_REPLACEMENTS:
        ...
        if use_anthropic_protocol:
            ...
            if settings.get('enable_tool_inject', False):
                ...
                if not is_pure_enabled and _system_portion_end > 0:
                    ...
                    if settings.get('enable_planned_tools', False):
                        _cost_rules = _apply_planned_cost_replacements(_cost_rules)
